...
def xiter(num_iter, ds1, avg_acc=0.02, max_acc=0.05):
    for x in range(1, num_iter + 1):
        ds1['SalesVXS'] = ds1['SalesVXS'] / ds1.groupby(['WeekDP', 'Category'])['SalesVXS'].transform(sum) * ds1['SalesVTP']
        ds1['Sum1'] = ds1.groupby(['Category', 'Store'])['SalesVXS'].transform(sum)
        ds1['SalesVXS'] = ds1['SalesVXS'] / ds1['Sum1'] * ds1['SalesVTS']
        ds1['Sum2'] = ds1.groupby(['WeekDP', 'Category'])['SalesVXS'].transform(sum)
        acc1 = ds1[['Category', 'SalesVTP', 'Sum2']].drop_duplicates()
    ...
    return ds1[['Category', 'Store', 'WeekDP', 'SalesVXS']]
